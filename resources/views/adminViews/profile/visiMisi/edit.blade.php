...
@endsection
        <div class="mt-5">
            ...
            <form action="{{ route('visiMisi.update', ['visimisi' => $visiMisi->uuid]) }}" class="forms-sample mt-5" method="POST">
                @csrf
                @method('patch')
                <div class="form-group">
                    <label for="exampleInputName1" class=" @error('title') text-danger @enderror">Judul</label>
                    <input type="text" class="form-control @error('title') is-invalid @enderror" id="exampleInputName1" name="title" value="{{ old('title') ? old('title') : $visiMisi->title }}">
                    @error('title')
                ...
                <div class="form-group">
                    ...
                    <textarea style="height: 200px;" class="form-control   @error('content') is-invalid @enderror" name="content">{!! old('content') ? old('content') : $visiMisi->content !!}</textarea>
                    @error('content')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
                </div>
